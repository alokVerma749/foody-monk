			...
			<nav className='max-w-7xl mx-auto px-4 sm:px-6 lg:px-8'>
				<div className='flex items-center justify-between h-16'>
					<div className='flex items-center'>
						...
						<div className='hidden md:block'>
							<div className='ml-10 flex items-baseline space-x-4'>
								<Link
									to={"/"}
									className='text-white px-3 py-2 text-base font-bold hover:font-black transition-all ease-in duration-300'
								>
									Home
								</Link>
								<Link
					...
					<div className='-mr-2 flex md:hidden'>
						<button
							onClick={toggleMenu}
							type='button'
							className='inline-flex items-center justify-center p-2 rounded-md text-white hover:text-gray-200 hover:bg-red-500 focus:outline-none focus:bg-red-500 focus:text-gray-200'
						>
							<svg
				...
				<div className={`${isMenuOpen ? "block" : "hidden"} md:hidden`}>
					<div className='px-2 pt-2 pb-3 space-y-1 sm:px-3'>
						...
						<Link to={"/cart"} className='text-white block px-3 py-2 rounded-md text-base font-medium'>
							Cart-{cartItems.length}
						</Link>
