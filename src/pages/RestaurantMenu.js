...
const RestaurantMenu = () => {
  ...
  const { name, city, locality, cuisines, avgRating, costForTwo, totalRatingsString, cloudinaryImageId } = resInfo ? resInfo?.cards[2]?.card?.card?.info : "null"

  let { itemCards } = resInfo ? resInfo?.cards[4]?.groupedCard?.cardGroupMap?.REGULAR?.cards[1]?.card?.card : []
  if (!itemCards) {
